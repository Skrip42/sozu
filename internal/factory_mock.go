...
//
//	mockgen -typed -destination=factory_mock.go -source=factory.go -package=internal
//
...
type MockFactory[V any] struct {
	...
	recorder *MockFactoryMockRecorder[V]
}
...
func NewMockFactory[V any](ctrl *gomock.Controller) *MockFactory[V] {
	mock := &MockFactory[V]{ctrl: ctrl}
	mock.recorder = &MockFactoryMockRecorder[V]{mock}
	return mock
}
...
func (mr *MockFactoryMockRecorder[V]) Create(ctx, inputCh, flushCh, beforeSend, afterSend, beforeFlush, afterFlush, capacity, cancel any) *MockFactoryCreateCall[V] {
	...
	call := mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "Create", reflect.TypeOf((*MockFactory[V])(nil).Create), ctx, inputCh, flushCh, beforeSend, afterSend, beforeFlush, afterFlush, capacity, cancel)
	return &MockFactoryCreateCall[V]{Call: call}
}
